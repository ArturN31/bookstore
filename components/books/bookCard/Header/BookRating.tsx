...
export const BookRating = ({ reviews }: { reviews: ReviewDB[] }) => {
    const bookRatings = reviews.map((review) => review.rating);
    const ratingsTotal = bookRatings.reduce((total, value) => total + value, 0);

    const rating =
        bookRatings.length > 0 ? (ratingsTotal / bookRatings.length).toPrecision(3) : '0.00';

    return (
        <p className="flex items-center justify-end select-none">
            <span
                data-testid={`book-rating-${reviews[0]?.book_id ?? 'empty'}`}
