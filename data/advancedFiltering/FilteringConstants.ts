...
export type FilteringTypes = {
    ...
    SALES: NonNullable<BookRow['sales_count']>[];
    STOCK: NonNullable<BookRow['stock_quantity']>[];
};
...
export const NUMERIC_CATEGORIES: (keyof FilteringTypes)[] = ['PAGES', 'PRICES', 'SALES', 'STOCK'];
...
export const getFilteringConstants = async (): Promise<FilteringTypes> => {
    ...
    const { data, error } = await supabase
        .from('books')
        .select(
            'author, format, genre, page_count, price, publication_date, publisher, sales_count, stock_quantity',
        );
    ...
    const sales = new Set<NonNullable<BookRow['sales_count']>>();
    const stock = new Set<NonNullable<BookRow['stock_quantity']>>();
    ...
    (data as BookFilterRow[]).forEach((book) => {
        ...
        if (book.sales_count !== null) sales.add(book.sales_count);
        if (book.stock_quantity !== null) stock.add(book.stock_quantity);
    });
